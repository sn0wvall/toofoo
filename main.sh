...
printHelp(){											# Output the help information to the user
    ...
    cat <<-EOF
		...
		show|print
			- Parameters: [today|[date]|all|up]
			- Shows events on the specified day
			e.g. toofoo show date 12/12/20

		new|add
			- Parameters: n/a
			- Queries user for new event and date, as well as information for recurring events
			- Date formats are those listed in date(1): e.g. 5 March 2019, 5/7/19
		
		del|rm
			- parameters: Event Name or Number
			- Queries user for event to delete, or deletes argument passed in
...
printEvents(){											# Output events to the user
	...
	case $1 in
		...
		all)	echo "SHOWING ALL EVENTS"; echo 
		...
		;;
		*|up)	echo "SHOWING EVENTS FROM TODAY, $date"; echo

			while [ $x -lt ${#calFile[@]} ]; do
				((number=$y-$w))
				nowDateRaw=$(mathable $date)
				nowDate=$(date -d "$nowDateRaw" +%s)
				testDateRaw=$(mathable "${calFile[$y]}")
				testDate=$(date -d "$testDateRaw" +%s)
				test $nowDate -le $testDate && 	
				echo "$number ${calFile[$x]}, on ${calFile[$y]}" && found=true && ((w=w+1))
...
date=$(date +"$calFormat")									# After configuration file path is generated by confFileGenerate, source it for use in the script
