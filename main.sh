...
newEvent(){											# Generate new events and write them out to the calendar file
	...
	test -z "${calFile[0]}" && echo "\"$eventDetailsCreate\"" >> "$calFileDest" && echo "$eventDateCreate" >> "$calFileDest"

	while [ $y -lt ${#calFile[@]} ]; do
		lineDate=$(date -d "${calFile[$y]}" "+%s"	2>/dev/null)
		userDate=$(date -d "$eventDateCreate" "+%s"	2>/dev/null)
		echo $lineDate $userDate $y $x 

		test $userDate -lt $lineDate && sed -i "$y i\\$eventDateCreate" "$calFileDest" && sed -i "$y i\\\"$eventDetailsCreate\"" "$calFileDest" && return 0
